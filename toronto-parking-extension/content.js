...
// it will fail fast. The background handles reloading on timeout.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      ...
      if (termsButton) {
        ...
        await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (800 - 200 + 1)) + 200));
        termsButton.click();
        await new Promise((r) =>
      ...
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (800 - 200 + 1)) + 200));
      lookupButton.click();
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (900 - 400 + 1)) + 400));
      ...
      ticketInput.click();
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (400 - 100 + 1)) + 100));
      ticketInput.value = '';
      for (const c of violationNum) {
        ticketInput.value += c;
        ticketInput.dispatchEvent(new Event('input', { bubbles: true }));
        await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (200 - 80 + 1)) + 80));
      }
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (400 - 100 + 1)) + 100));
      ...
      for (const xpath of ticketInputErrorXpaths) {
        const iterator2 = document.evaluate(xpath, document, null, XPathResult.ANY_TYPE, null);
      ...
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (800 - 200 + 1)) + 200));
      submitButton.click();
      await new Promise((r) => setTimeout(r, Math.floor(Math.random() * (1500 - 800 + 1)) + 800));
      ...
      if (cells.length < 6) throw new Error('Unexpected table format: fewer than 6 columns');
      ...
      // If the error is table‐related, retry up to maxAttempts
      if (
        attempt < maxAttempts - 1 &&
        (err.message.includes('No ticket table') || err.message.includes('Unexpected table format'))
      ) {
        continue;
      }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'runLookup') {
    ...
    return true; // Keep sendResponse open until async work completes
  }
});
